...
export default async function HomePage() {
  ...
  // Hämta profil och egna föremål om inloggad
  let profile = null
  let myItems: any[] = []
        ...
        <div className="max-w-5xl mx-auto px-4 py-10">
          ...
          {items && items.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {items.map(item => <AuctionCard key={item.id} item={item as Item} />)}
            </div>
          ) : (
      ...
      <div className="min-h-screen bg-stone-50">
        ...
        <div className="bg-[#1a1208] px-4 py-8">
          <div className="max-w-5xl mx-auto">
            <p className="text-[#8B6914] text-xs tracking-widest uppercase mb-1">Välkommen tillbaka</p>
            <h1 className="text-2xl text-[#D4AF37] font-medium">{profile.full_name}</h1>
            <p className="text-[#8B6914] text-sm mt-1">{items?.length || 0} aktiva auktioner att buda på</p>
          </div>
        </div>
        <div className="max-w-5xl mx-auto px-4 py-8">
